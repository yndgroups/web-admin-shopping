...
const createLayoutComponentsRelation = (): AnyObject => {
  ...
  Object.keys(layouts).forEach(key => {
    const routerPath = key.replace(/.vue|\/src\/views/g, '')
...
const getCompInfo = (): any => {
  const pages = import.meta.glob('@/views/**') as any
  const fns = new Array<any>()
  const keys = new Array<string>()
  const comps = new Array<any>()
  Object.keys(pages).forEach(key => {
    const comp = pages[key]
    keys.push(key.replace(/.vue|\/src\/views/g, ''))
    comps.push(comp)
...
const init = async (app: App<Element>) => {
  ...
  router.beforeEach((to, form, next) => {
    ...
    if (isMobile() && to.query.vconsole) {
      ...
      if (!d3) {
        ...
        d3Script.src = '/js/vconsole.min.js'
  ...
  router.afterEach(to => {
    Nprogress.done()
    console.log('router -> fullPath:', to.fullPath, 'location -> href:', window.location.href)
    const meta: AnyObject = to.meta || {}
    const { title, pv } = meta
    useTitle(title || '')
    if (pv) {
      Logger.log('执行某个pv统计操作')
    }
  })
