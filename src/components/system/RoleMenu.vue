        ...
        >
          ...
          >
            <template #title="{ name, type }">
              ...
              <!-- <span class="text-gray pd-l10"> - {{ powerSign }}</span> -->
            </template>
          </a-tree>
        </div>
      </div>
...
const columns = [
  ...
  { title: '菜单地址', dataIndex: 'url', key: 'url' },
  {
    title: '权限值',
    dataIndex: 'powerSign',
    /* width: '30%',*/ key: 'powerSign',
  },
  { title: '排序', dataIndex: 'sortBy', width: 60, key: 'sortBy' },
  { title: '操作', width: 250, key: 'operation' },
]
...
const saveData = async (data: any[]) => {
  const { code, msg } = await apis.postJSON(apis.addMenuToRole, {
    data: data,
  })
  if (code === 1) {
    message.success(msg)
...
const getRoleMenuIds = async () => {
  console.log('getRoleMenuIds', props.roleItem)
  state.loading = true
  let { data, code } = await apis.getJSON(`${apis.findMenuInfoByRoleId}/${props.roleItem.roleId}`)
  if (code === 1) {
...
// .ant-table .menu-table th,
// .ant-table .menu-table td {
//   padding: 2px;
// }
